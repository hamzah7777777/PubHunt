-- PubHunt anagram challenge: per-team anagram answers + marking.
-- Run this in the Supabase SQL editor (after schema.sql).
--
-- Teams see 8 anagrams of video game names and submit the game for each
-- (resubmitting overwrites and clears any existing mark). Admins mark each
-- answer: 1 point per correct game.

create table if not exists anagram_answers (
  id uuid primary key default gen_random_uuid(),
  team_id uuid not null references teams(id) on delete cascade,
  anagram_number int not null check (anagram_number between 1 and 8),
  answer text not null,
  -- null = not marked yet, true/false = marked by an admin
  is_correct boolean,
  submitted_at timestamptz not null default now(),
  unique (team_id, anagram_number)
);

create index if not exists anagram_answers_team_id_idx on anagram_answers(team_id);
...
alter table anagram_answers enable row level security;
...
grant execute on function submit_anagram_answer(uuid, int, text) to anon;

drop function if exists get_team_anagram_answers(uuid);
create or replace function get_team_anagram_answers(p_team_id uuid)
returns table (
  anagram_number int,
  answer text,
  is_correct boolean
...
grant execute on function get_team_anagram_answers(uuid) to anon;
